...
func _on_animate_start(target: Node) -> void:
	...
	match transform_target:
		TransformTarget.POSITION:
			JuiceLogger.log_capture(self, _get_domain_tag(), "rate",
				{"position_rate": position_rate, "unit": PositionIn3D.keys()[position_unit]},
				debug_enabled)
		TransformTarget.ROTATION:
			JuiceLogger.log_capture(self, _get_domain_tag(), "rate",
	...
	if bound_enabled:
		JuiceLogger.log_capture(self, _get_domain_tag(), "bound",
			...
			"mode": BoundMode.keys()[bound_mode],
...
func _restore_to_natural(target: Node) -> void:
	...
	match transform_target:
		...
		TransformTarget.ROTATION: acc_log = Vector3(rad_to_deg(_accumulated_rotation.x), rad_to_deg(_accumulated_rotation.y), rad_to_deg(_accumulated_rotation.z))
		TransformTarget.SCALE:    acc_log = _accumulated_scale
	JuiceLogger.log_info(self, _get_domain_tag(),
			"restore_to_natural: hold=%s accumulated=%s pos_d=%s rot_d=%s scale_d=%s" % [
...
func _apply_effect(progress: float, target: Node) -> void:
	...
	match transform_target:
		...
		TransformTarget.ROTATION: accumulated_log = Vector3(rad_to_deg(_accumulated_rotation.x), rad_to_deg(_accumulated_rotation.y), rad_to_deg(_accumulated_rotation.z))
...
func _check_bounds() -> void:
	...
	match transform_target:
		...
		TransformTarget.ROTATION: accumulated_at_bound = Vector3(rad_to_deg(_accumulated_rotation.x), rad_to_deg(_accumulated_rotation.y), rad_to_deg(_accumulated_rotation.z))
