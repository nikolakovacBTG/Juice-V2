...
## finds the active Camera2D's CameraJuiceUtility and applies the offset.
...
# DOES NOT: Cache the utility reference — re-discovers each frame so mid-
#            animation camera switches work correctly.
#
# SETUP: Add a CameraJuiceUtility node as a direct child of your Camera2D.
...
## Needed to correctly undo our contribution on cleanup.
...
func _apply_effect(progress: float, _target: Node) -> void:
	# Re-discover utility every frame — handles mid-animation camera switches.
...
func _apply_position(util: CameraJuiceUtility, progress: float) -> void:
	# Convert viewport % to pixels. Resolution-independent.
...
func _apply_rotation(util: CameraJuiceUtility, progress: float) -> void:
	# Map degrees -> radians on Z-axis (Camera2D rotates on Z only).
...
func _remove_contribution() -> void:
	# Try to undo what we wrote. If utility is gone (camera freed), just reset.
	var util := _find_utility()
...
## Finds the active Camera2D's CameraJuiceUtility via the host node's viewport.
## Re-discovers every call -- O(1) viewport + O(children) type scan.
func _find_utility() -> CameraJuiceUtility:
	...
	if debug_enabled:
		push_warning("[Camera2DJuiceEffect] No CameraJuiceUtility found on Camera2D '%s'. Add one as a direct child." % cam.name)
	return null
