...
func _create_overlay_comp_cover() -> Node:
	# Create a runtime ScreenOverlayJuiceComp for the cover phase
	var comp := ScreenOverlayJuiceComp.new()
	comp.name = "_TransitionOverlayComp"
	comp.process_mode = Node.PROCESS_MODE_ALWAYS

	# Configure overlay appearance
	comp.overlay_color = overlay_color
	comp.max_alpha = 1.0
	comp.direction = ScreenOverlayJuiceComp.OverlayDirection.TO_COLOR
	...
	# Configure timing from the utility's Animate In group (= cover timing)
	comp.duration_in = cover_duration
	comp.transition_in = cover_transition
	comp.ease_in = cover_ease
	comp.custom_curve_in = cover_curve

	# Sequencer pattern: trigger on ready so it plays immediately when we call animate_in()
	...
	# Add as child of handler (lives on root, survives scene change)
	add_child(comp)

	# Wait one frame for _ready() to fire on the comp
	await get_tree().process_frame

	if debug_enabled:
		print("[TransitionHandler] Cover overlay comp created (duration=%.2f)" % cover_duration)

	return comp


func _configure_overlay_comp_reveal(comp: Node) -> void:
	# Reconfigure the existing overlay comp for the reveal phase
	# Change direction to TO_CLEAR (opaque → transparent)
	comp.direction = ScreenOverlayJuiceComp.OverlayDirection.TO_CLEAR

	# Use the utility's Animate Out group (= reveal timing)
	comp.duration_in = reveal_duration
	comp.transition_in = reveal_transition
	comp.ease_in = reveal_ease
	comp.custom_curve_in = reveal_curve

	if debug_enabled:
		print("[TransitionHandler] Reveal overlay comp configured (duration=%.2f)" % reveal_duration)
