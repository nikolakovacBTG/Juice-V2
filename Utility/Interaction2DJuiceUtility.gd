...
##
## SYSTEM: Juice Utility (addons/juice/Utility/)
##
## DOES NOT:
...
## - Handle visual effects directly (JuiceCompBase nodes handle effects)
##
## CONNECTIONS:
## - JuiceCompBase children/siblings: auto-connect to native Area2D signals
##   via JuiceCompBase's auto-connect logic. No wrapper signals needed.
...
func _get_configuration_warnings() -> PackedStringArray:
	...
	if mode == Mode.INTERACTABLE:
		if not has_root_shape:
			warnings.append("No CollisionShape2D with a shape found. Mouse interaction requires a collision shape.")
		for i in range(mini(action_count, _action_presets.size())):
			if _action_presets[i] == InputActionPreset.CUSTOM:
		...
		if check_presence_in_trigger_zone:
			...
			if tz == null:
				warnings.append("No trigger zone child found. One will be auto-created, or add an Area2D child manually.")
			else:
				...
				if not has_zone_shape:
					warnings.append("Trigger zone has no CollisionShape2D with a shape.")
				if (collision_layer & tz.collision_mask) != 0 or (tz.collision_layer & collision_mask) != 0:
					warnings.append("This Area2D and its trigger zone share collision layers/masks. They may detect each other.")
	elif mode == Mode.TRIGGER_ZONE:
		if not has_root_shape:
			warnings.append("No CollisionShape2D with a shape found. Trigger zone requires a collision shape.")
	...
	if not has_juice and get_parent():
		for sibling in get_parent().get_children():
			...
			if sibling_script and sibling_script.get_global_name() and sibling_script.get_global_name().ends_with("JuiceUtility"):
				...
				break
	if not has_juice:
		warnings.append("No JuiceCompBase children or siblings found.")
...
func _find_trigger_zone() -> Area2D:
	for child in get_children():
		if child is Area2D and child.has_meta(_TRIGGER_ZONE_META):
			return child
	return null
