## TestProgress2D.gd
## ============================================================================
## WHAT: Tests for Progress2DJuiceEffect across position, rotation, scale.
...
## Creates a minimal Juice2D + Node2D rig with a Progress2DJuiceEffect.
func _create_progress_rig(
	label: String,
	target_type: int = Progress2DJuiceEffect.TransformTarget.POSITION,
	...
	var effect := Progress2DJuiceEffect.new()
...
func test_position_accumulates() -> void:
	...
	var rig := await _create_progress_rig("prog_pos_2d",
		Progress2DJuiceEffect.TransformTarget.POSITION,
...
func test_rotation_accumulates() -> void:
	...
	var rig := await _create_progress_rig("prog_rot_2d",
		Progress2DJuiceEffect.TransformTarget.ROTATION,
		Vector2(100.0, 0.0), 90.0, Vector2(0.5, 0.5), 0.1)
	var target: Node2D = rig[0]
	var juice: Juice2D = rig[1]
...
func test_scale_accumulates() -> void:
	...
	var rig := await _create_progress_rig("prog_scale_2d",
		Progress2DJuiceEffect.TransformTarget.SCALE,
		Vector2(100.0, 0.0), 90.0, Vector2(0.5, 0.5), 0.1)
	var target: Node2D = rig[0]
	var juice: Juice2D = rig[1]
...
func test_hold_on_stop_true() -> void:
	...
	var target: Node2D = rig[0]
	var juice: Juice2D = rig[1]
	var effect: Progress2DJuiceEffect = rig[2]
...
func test_bound_reverse() -> void:
	...
	var effect := Progress2DJuiceEffect.new()
	effect.transform_target = Progress2DJuiceEffect.TransformTarget.POSITION
	effect.position_rate = Vector2(200.0, 0.0)  # Fast — hits bound quickly
	effect.trigger_behaviour = JuiceEffectBase.TriggerBehaviour.PLAY_IN_ONLY
	effect.duration_in = 0.05
	effect.bound_enabled = true
	effect.bound_behaviour = Progress2DJuiceEffect.BoundBehaviour.REVERSE
...
func test_needs_sustain() -> void:
	...
	assert_true(effect._needs_sustain(),
		"Progress2DJuiceEffect must return true from _needs_sustain()")
