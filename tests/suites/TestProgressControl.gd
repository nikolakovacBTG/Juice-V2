## TestProgressControl.gd
## ============================================================================
## WHAT: Tests for ProgressControlJuiceEffect across position, rotation, scale.
...
func _create_progress_rig(
	label: String,
	target_type: int = ProgressControlJuiceEffect.TransformTarget.POSITION,
	...
	var effect := ProgressControlJuiceEffect.new()
...
func test_position_accumulates() -> void:
	var rig := await _create_progress_rig("prog_pos_ctrl",
		ProgressControlJuiceEffect.TransformTarget.POSITION,
...
func test_rotation_accumulates() -> void:
	var rig := await _create_progress_rig("prog_rot_ctrl",
		ProgressControlJuiceEffect.TransformTarget.ROTATION,
		Vector2(100.0, 0.0), 90.0, Vector2(0.5, 0.5), 0.1)
	var target: Control = rig[0]
	var juice: JuiceControl = rig[1]
...
func test_scale_accumulates() -> void:
	var rig := await _create_progress_rig("prog_scale_ctrl",
		ProgressControlJuiceEffect.TransformTarget.SCALE,
		Vector2(100.0, 0.0), 90.0, Vector2(0.5, 0.5), 0.1)
	var target: Control = rig[0]
	var juice: JuiceControl = rig[1]
...
func test_hold_on_stop_true() -> void:
	...
	var target: Control = rig[0]
	var juice: JuiceControl = rig[1]
	var effect: ProgressControlJuiceEffect = rig[2]
...
func test_bound_reverse() -> void:
	...
	var effect := ProgressControlJuiceEffect.new()
	effect.transform_target = ProgressControlJuiceEffect.TransformTarget.POSITION
	effect.position_rate = Vector2(200.0, 0.0)
	effect.trigger_behaviour = JuiceEffectBase.TriggerBehaviour.PLAY_IN_ONLY
	effect.duration_in = 0.05
	effect.bound_enabled = true
	effect.bound_behaviour = ProgressControlJuiceEffect.BoundBehaviour.REVERSE
...
func test_needs_sustain() -> void:
	...
	assert_true(effect._needs_sustain(),
		"ProgressControlJuiceEffect must return true from _needs_sustain()")
