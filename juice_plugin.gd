## juice_plugin.gd
## ============================================================================
## WHAT: Master EditorPlugin for the Juice addon.
## WHY: Single entry point for all Juice editor features. Currently provides
##      transport controls (play/pause/stop/scrub) for previewing JuiceCompBase
##      effects in the editor without running the game.
## SYSTEM: Juice System (addons/juice/)
## DOES NOT: Implement effects (that's JuiceCompBase subclasses), manage
##           animation state (that's JuicePreviewDirector), or handle undo/redo.
## ============================================================================
##
## CONNECTIONS:
## - EditorInterface.get_selection(): Listens to selection_changed to detect
##   when a JuiceCompBase is selected/deselected.
## - JuicePreviewDirector: Delegates all preview logic. Connects to
##   time_updated and state_changed signals.
## - Editor main screen: Transport UI is added as a child so it overlays
##   the 2D/3D viewport area.
## ============================================================================

@tool
extends EditorPlugin

const PreviewDirectorScript := preload("res://addons/juice/Editor/JuicePreviewDirector.gd")
const DockScene := preload("res://addons/juice/Editor/JuiceTransportDock.tscn")
...
## Typed as Node because class_name isn't available until the plugin is enabled.
var _director: Node
...
var _non_scrubbable_label: Label
...
func _enter_tree() -> void:
	...
	# Initial check in case something is already selected
	_on_selection_changed()


## Called by Godot BEFORE saving the project (Ctrl+S, tab switch, etc.).
## Temporarily snaps all previewed comps to natural state so the .tscn gets
...
## NOTE: _save_external_data() runs AFTER save. _apply_changes() runs BEFORE.
func _apply_changes() -> void:
	if _director and (_director._preview_comps.size() > 0 or _director._chain_comps.size() > 0):
...
func _build_ui() -> void:
	...
	# Anchored bottom-right of the viewport overlay (same pattern as Brackey's).
	...
	# Non-scrubbable message — shown when chain has multiple comps
	_non_scrubbable_label = Label.new()
	_non_scrubbable_label.text = "Only single juice comps are scrubbable"
	_non_scrubbable_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	_non_scrubbable_label.add_theme_font_size_override("font_size", 10)
	_non_scrubbable_label.add_theme_color_override("font_color", Color(1.0, 0.8, 0.3, 0.8))
	_non_scrubbable_label.visible = false
	content.add_child(_non_scrubbable_label)
...
func _setup_director() -> void:
	_director = PreviewDirectorScript.new()
...
func _on_selection_changed() -> void:
	...
	var juice_comp: JuiceCompBase = null
	if selection.size() == 1 and selection[0] is JuiceCompBase:
		juice_comp = selection[0] as JuiceCompBase

	if juice_comp and juice_comp._supports_editor_preview() and juice_comp.get_parent() != null:
		_director.select(juice_comp)
		_update_title(juice_comp)
		_update_out_button_visibility(juice_comp)
		if was_playing:
			_director.play()
		_dock.visible = true
	else:
...
func _on_state_changed() -> void:
	...
	# Scrub slider: hide if chain is non-scrubbable (multiple comps)
	var scrubbable: bool = _director.is_scrubbable
	_scrub_slider.visible = scrubbable
	_non_scrubbable_label.visible = not scrubbable
	if scrubbable:
...
## Update the title label with the selected comp's name.
func _update_title(comp: JuiceCompBase) -> void:
	_title_label.text = "Juice: %s" % str(comp.name)


## Hide the OUT button if the comp has no OUT phase.
func _update_out_button_visibility(comp: JuiceCompBase) -> void:
	var has_out := comp.trigger_behaviour == JuiceCompBase.TriggerBehaviour.PLAY_IN_AND_OUT \
		or comp.trigger_behaviour == JuiceCompBase.TriggerBehaviour.TOGGLE_IN_AND_OUT \
		or comp.trigger_behaviour == JuiceCompBase.TriggerBehaviour.PLAY_OUT_ONLY
