...
This document has been **updated to reflect the actual implementation** in Transform2DJuiceEffect.gd.
Key changes from the original design:

### ✅ **Implementation Improvements**
- **Unified enum**: `TransformReference` instead of separate per-type enums (better design)
- **Clearer names**: `OWN_SIZE`, `PARENT_SIZE`, `VIEWPORT_SIZE` instead of `FRACTION_*`
- **Extra feature**: `IN_EDITOR` capture mode for WYSIWYG editing
- **Better defaults**: `to_rotation_degrees: 15.0` instead of `0.0`

### ⚠️ **Missing Feature**
- **Configuration warning**: "Self + Self" warning triangle not implemented (minor UX gap)

### 📊 **Fidelity**: ~95% - The implementation faithfully follows the design with minor improvements.
...
# --- Reference Types (UNIFIED across all transform types) ---

## Reference type for From/To axes (shared by Position, Rotation, and Scale)
## Implementation uses unified enum instead of separate per-type enums
...
enum PositionIn {
    ...
    OWN_SIZE,         # Position as multiple of object's own size (implementation name)
    PARENT_SIZE,      # Position as multiple of parent's size (implementation name)
    VIEWPORT_SIZE     # Position as multiple of viewport size (implementation name)
}
...
# IMPLEMENTATION NOTE: Uses unified TransformReference enum instead of separate per-type enums
var from_reference: int = TransformReference.SELF:  # Default: SELF
    set(value):
        from_reference = value
        notify_property_list_changed()

var to_reference: int = TransformReference.CUSTOM:  # Default: CUSTOM
    set(value):
        to_reference = value
        notify_property_list_changed()
...
var from_position_in: int = PositionIn.OWN_SIZE   # IMPLEMENTATION: Default changed from PIXELS
var to_position: Vector2 = Vector2.ZERO
var to_position_in: int = PositionIn.OWN_SIZE     # IMPLEMENTATION: Default changed from PIXELS

# Rotation (2D: single float in degrees. 3D: Vector3 + RotationUnit)
var from_rotation_degrees: float = 0.0       # 2D only
var to_rotation_degrees: float = 15.0        # IMPLEMENTATION: Default changed from 0.0 to 15.0
...
func _get_rotation_properties() -> Array[Dictionary]:
    ...
    if from_reference == TransformReference.CUSTOM or to_reference == TransformReference.CUSTOM:
...
From
├─ from_reference: [Custom ▼]
├─ from_position_in: [Viewport Size ▼]
└─ from_position: [-0.5, 0]
...
func _convert_to_world_pixels(position: Vector2, position_in: PositionIn) -> Vector2:
    match position_in:
        PositionIn.PIXELS:
            return position
        PositionIn.OWN_SIZE:          # IMPLEMENTATION: Updated name
            var size := _infer_node2d_size(_target_node as Node2D)
            return Vector2(position.x * size.x, position.y * size.y)
        PositionIn.PARENT_SIZE:       # IMPLEMENTATION: Updated name
            var size := _infer_parent_size()
            return Vector2(position.x * size.x, position.y * size.y)
        PositionIn.VIEWPORT_SIZE:     # IMPLEMENTATION: Updated name
...
- **IMPLEMENTATION NOTE**: Configuration warning for "Self + Self" is **NOT IMPLEMENTED**.
The design document specifies:
- **Self + Self**: "Both From and To reference Self — animation will have no visible effect."

However, the actual implementation does not include `_get_configuration_warnings()` override.
This is a minor missing UX feature that could be added in a future update.
