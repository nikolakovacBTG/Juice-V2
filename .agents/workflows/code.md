...
The `# WHY:` block must define the class's architectural purpose and the constraints it enforces. Do not explain migration history (e.g. no "V0", "ported", "refactored"). Transform legacy comments into pure structural rationale. Explain *why* the architecture demands a specific approach, without referencing past versions.
