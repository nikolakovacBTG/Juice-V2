...
description: Enforces mandatory registration of new Juice effects in domain recipes. Prevents "orphaned" effects that don't appear in the Godot inspector.
---

# Registration Guard Skill

**When to use:** Before declaring any effect port or creation as "done".

## The Protocol

A Juice effect is not complete until it is registered in its respective Domain Recipe. If an effect is not registered, it will not appear as an option in the Godot Inspector when adding effects to a `JuiceControl`, `Juice2D`, or `Juice3D` node.

### 1. Identify the Target Recipes
Depending on the domains ported, check:
- **Control**: `addons/Juice_V2/Base Classes/JuiceControlRecipe.gd`
- **2D**: `addons/Juice_V2/Base Classes/Juice2DRecipe.gd`
- **3D**: `addons/Juice_V2/Base Classes/Juice3DRecipe.gd`
...
grep "[EffectName]" "addons/Juice_V2/Base Classes/Juice2DRecipe.gd"
```
