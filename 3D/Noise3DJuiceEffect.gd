...
## Unit for position amplitude: absolute World Units, or relative to Own Size or Parent Size.
...
## When enabled, all axes use the same noise sample for uniform scaling.
...
## Per-axis speed multiplier for position noise. Allows different oscillation rates on each axis.
var position_axis_speed: Vector3 = Vector3(1.0, 1.0, 1.0)
## Per-axis speed multiplier for rotation noise. Allows different oscillation rates on each axis.
