...
# =============================================================================
# WHAT: One shake target slot — property path + type-matched amplitude.
...
#       array to shake with an independent magnitude.
...
## euler axes give direct per-axis rotation shake control. Converted at apply time.
...
## Threshold for bool properties. Property is written as (shake_val > flip_threshold)
## each frame — it toggles on every crossing. Range matches shake output [-1, 1].
## Default 0.0 = midpoint, giving roughly 50/50 on/off distribution.
...
## Emits Target (path fields) then the amplitude or State A/B fields matching _detected_type.
func _get_property_list() -> Array[Dictionary]:
	...
	if is_discrete:
		_emit_state_group(props, false)
		_emit_state_group(props, true)
